...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    // Apuntamos a la subcolección profunda exacta para traer la agenda original
    const unsubContacts = onSnapshot(collection(db, 'artifacts', 'fresenius-hub-v1', 'public', 'data', 'contacts'), 
      s => setContacts(s.docs.map(d => ({id: d.id, ...d.data()}))), 
      e => { console.error(e); setError("Permisos denegados en Contactos (Revisa tus reglas)"); }
    );
    // Nos aseguramos que el lector en tiempo real también escuche la ruta raíz
    const unsubOrders = onSnapshot(collection(db, 'service_orders'), 
      s => setServiceOrders(s.docs.map(d => ({id: d.id, ...d.data()}))), 
      e => { console.error(e); setError("Permisos denegados en Órdenes (Revisa tus reglas)"); }
    );
        ...
        <div className="flex-1 overflow-y-auto scroll-smooth overscroll-none overflow-x-hidden">
          ...
          {screen === 'menu' && (
            <MenuScreen 
              ...
              addContact={c => addDoc(collection(db, 'artifacts', 'fresenius-hub-v1', 'public', 'data', 'contacts'), {...c, addedBy: user?.uid})} 
              deleteContact={id => deleteDoc(doc(db, 'artifacts', 'fresenius-hub-v1', 'public', 'data', 'contacts', id))} serviceOrders={serviceOrders} sheetOrders={sheetOrders} setSheetOrders={setSheetOrders} equipment={equipment} 
