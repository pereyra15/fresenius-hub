...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    // Apuntamos a la subcolección específica "public -> data -> contacts" para traer la agenda original
    const unsubContacts = onSnapshot(collection(db, 'public', 'data', 'contacts'), 
        ...
        <div className="flex-1 overflow-y-auto scroll-smooth overscroll-none overflow-x-hidden">
          ...
          {screen === 'menu' && (
            <MenuScreen 
              ...
              addContact={c => addDoc(collection(db, 'public', 'data', 'contacts'), {...c, addedBy: user?.uid})} 
              deleteContact={id => deleteDoc(doc(db, 'public', 'data', 'contacts', id))} serviceOrders={serviceOrders} sheetOrders={sheetOrders} setSheetOrders={setSheetOrders} equipment={equipment} 
