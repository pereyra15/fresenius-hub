...
const MenuButton = ({ label, subScreenId, svgIcon, setMenuSubScreen, menuSubScreen, navigate }) => {
  const isActive = menuSubScreen === subScreenId;
  return (
    <button
      onClick={() => navigate('menu', subScreenId)}
...
const LandingScreen = ({ navigate }) => (
  <div className="flex flex-col gap-6 animate-fadeIn pb-10">
    ...
    <button onClick={() => navigate('register')} className="p-8 bg-white border-2 border-gray-100 rounded-[2.5rem] shadow-xl hover:border-blue-500 transition-all group flex items-center gap-6">
    ...
    <button onClick={() => navigate('login')} className="p-8 bg-blue-600 rounded-[2.5rem] shadow-xl hover:bg-blue-700 transition-all group flex items-center gap-6">
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'service_orders'), {
        ...reportForm, status: 'Abierto', createdAt: new Date().toISOString(), createdBy: userId, engineerName: loginForm.engineerName
      });
    ...
    } catch (e) { setError("Fallo al guardar reporte."); }
  ...
  const closeOrder = async (order) => {
    ...
    try {
      ...
      if (GOOGLE_SHEETS_URL) {
        await fetch(GOOGLE_SHEETS_URL, { 
          method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, 
          body: JSON.stringify({ action: 'UPDATE', firebaseId: order.id, ESTATUS: 'Cerrado' }) 
        });
      }
      setMessage("ORDEN FINALIZADA.");
    } catch (e) { setError("Error al sincronizar."); }
    finally { setClosingOrder(false); }
  };

  const fetchSpareParts = async (url, viewName) => {
    setLoadingSpares(true);
    navigate('menu', 'materialApoyo', 'SPAREPARTS', viewName); // Guardamos la navegación profunda
    try {
    ...
    } catch (e) { setError("Fallo en sincronización."); }
    ...
    switch (menuSubScreen) {
      case 'equipos':
        const filtered = equipment.filter(item => {
          ...
          return (eng.includes(resp) || resp.includes(eng)) && (item.serie + (item.modelo || '') + (item.cliente || '')).toLowerCase().includes(searchTerm.toLowerCase());
        });
        return (
          <div className="bg-white rounded-[2rem] shadow-sm border flex flex-col overflow-hidden animate-fadeIn">
            <div className="p-4 bg-gray-50 border-b sticky top-0 z-10"><input type="text" placeholder="Buscar..." className="w-full p-4 border rounded-2xl text-sm outline-none focus:ring-2 focus:ring-blue-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
                ...
                <tbody className="divide-y text-left">
                  {filtered.map(item => (
                    <tr key={item.id} onClick={() => setSelectedEquipment(item)} className="hover:bg-blue-50 cursor-pointer">
                      ...
                      <td className="p-4"><div className="font-black text-gray-800 uppercase text-[10px]">{item.modelo}</div><div className="text-[9px] text-gray-400 uppercase truncate max-w-[120px]">{item.cliente}</div></td>
                      <td className="p-4 text-center text-lg">{sheetOrders.some(so => so.serie === item.serie && so.status?.toLowerCase().includes('abierto')) ? '🔴' : '🟢'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        return (
          <div className="grid gap-5 animate-fadeIn pb-20">
            <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 mb-2">
              <h4 className="font-black text-blue-800 text-xs uppercase tracking-widest mb-1">Métricas</h4>
              <div className="flex gap-4">
                <div className="flex-1"><span className="block text-2xl font-black text-blue-900">{sheetOrders.filter(so => so.engineerName === loginForm.engineerName && so.status?.toLowerCase().includes('abierto')).length}</span><span className="text-[8px] font-black text-blue-500 uppercase">Abiertas</span></div>
                <div className="flex-1"><span className="block text-2xl font-black text-green-600">{sheetOrders.filter(so => so.engineerName === loginForm.engineerName && so.status?.toLowerCase().includes('cerrado')).length}</span><span className="text-[8px] font-black text-green-500 uppercase">Cerradas</span></div>
              </div>
            </div>
            <button onClick={() => navigate('menu', 'ordenesAsignadas')} className="p-10 bg-white border-2 border-gray-100 rounded-[2.5rem] font-black text-gray-800 shadow-xl flex flex-col items-center gap-3">📋 MIS ÓRDENES</button>
      ...
      case 'generarOS':
        return (
          <div className="p-6 bg-white border rounded-[2rem] overflow-y-auto animate-fadeIn text-left shadow-sm pb-20">
            ...
            <Input label="Hospital" name="cliente" value={reportForm.cliente} readOnly />
            ...
            <button onClick={() => setShowGenerateConfirm(true)} disabled={loadingReport} className="w-full bg-green-600 text-white font-black py-5 rounded-2xl mt-8 shadow-xl uppercase">{loadingReport ? 'Enviando...' : 'Generar Reporte'}</button>
          </div>
        );
        ...
        return (
          <div className="flex flex-col bg-white border rounded-[2rem] overflow-hidden animate-fadeIn pb-20">
            <div className="p-5 bg-purple-50 border-b flex flex-col gap-3">
                <div className="flex justify-between items-center"><h3 className="font-black text-purple-900 uppercase">Agenda</h3>{!showAddContactForm && (<button onClick={() => setShowAddContactForm(true)} className="bg-purple-600 text-white text-[10px] font-black py-2 px-5 rounded-full shadow-lg">Nuevo+</button>)}</div>
                {showAddContactForm && (
                  <div className="bg-white p-5 rounded-3xl border border-purple-200 shadow-xl animate-slideDown">
                    <div className="flex justify-between items-center mb-4 uppercase">
                      <h4 className="text-[10px] font-black text-purple-700">Registro</h4>
                      <button onClick={() => setShowAddContactForm(false)}>✕</button>
                    </div>
                    <div className="space-y-4">
                        <Input label="Nombre" placeholder="NOMBRE" value={newContact.name} onChange={e => setNewContact({...newContact, name: e.target.value})} />
                        ...
                        <Input label="Hospital" placeholder="HOSPITAL" value={newContact.unit} onChange={e => setNewContact({...newContact, unit: e.target.value})} />
                        <Input label="Email" placeholder="EMAIL" type="email" value={newContact.email} onChange={e => setNewContact({...newContact, email: e.target.value})} />
                        <button onClick={handleSaveContact} className="w-full bg-purple-600 text-white font-black py-4 rounded-xl uppercase">Guardar</button>
                    </div>
                  </div>
                )}
            </div>
      ...
      case 'materialApoyo':
        if (documentationSubScreen === 'SPAREPARTS') {
            ...
            return (
              <div className="flex flex-col bg-white border rounded-[2rem] overflow-hidden animate-fadeIn pb-20">
                ...
                <div className="flex-1 overflow-auto p-2 min-h-[400px]">
                  {loadingSpares ? <div className="text-center p-20 animate-pulse text-indigo-300 font-black">Cargando...</div> : (
                    <table className="w-full text-[9px] text-left border-collapse">
                      <thead className="bg-gray-50 sticky top-0">
                        <tr>{sparePartsData[0] && Object.keys(sparePartsData[0]).map(k => (<th key={k} className="p-2 border-b font-black text-gray-400 uppercase tracking-tighter">{k}</th>))}</tr>
                      </thead>
                                ...
                                {typeof val === 'string' && val.includes('drive.google.com') ? (
                                  <div className="flex flex-col gap-2">
                                    ...
                                    <div className="relative group w-16 h-16">
                                      {(() => {
                                        const driveId = val.match(/\/d\/([a-zA-Z0-9_-]+)/)?.[1];
                                        return driveId ? (
                                          <img 
                                            src={`https://googleusercontent.com/profile/picture/6${driveId}=w400`} 
                                            className="h-full w-full object-cover rounded-lg border shadow-sm group-hover:scale-[2.5] group-hover:z-50 transition-transform bg-white" 
                                            alt="Refacción"
                                            onError={(e) => {
                                              ...
                                              } else { e.target.src = 'https://placehold.co/100x100?text=Error'; }
                                            }}
          ...
          return (
            <div className="p-6 bg-white border rounded-[2rem] animate-fadeIn pb-20">
               <button onClick={goBack} className="mb-8 flex items-center gap-2 text-indigo-600 font-black uppercase text-[10px] tracking-widest"><span className="w-8 h-8 bg-indigo-50 rounded-lg flex items-center justify-center">←</span> Volver</button>
               <div className="grid gap-4">
                 <button onClick={() => fetchSpareParts('https://docs.google.com/spreadsheets/d/e/2PACX-1vS86FFjvfk8XJXF0bqgcyzAhADOQbtLDFH7JwFcFvqWxJHZugcqxGPky63hB65KJXRfChRXK_kapw3x/pub?gid=1919791847&single=true&output=csv', 'COMPOGUARD')} className="p-8 bg-indigo-50 border-2 border-indigo-100 rounded-[2rem] font-black text-indigo-700 hover:bg-indigo-100 transition-all flex flex-col items-center gap-2 shadow-sm"><span className="text-3xl">⚖️</span> COMPOGUARD</button>
                 <button onClick={() => fetchSpareParts('https://docs.google.com/spreadsheets/d/e/2PACX-1vS86FFjvfk8XJXF0bqgcyzAhADOQbtLDFH7JwFcFvqWxJHZugcqxGPky63hB65KJXRfChRXK_kapw3x/pub?gid=1546674811&single=true&output=csv', 'COMPOMAT G5')} className="p-8 bg-indigo-50 border-2 border-indigo-100 rounded-[2rem] font-black text-indigo-700 hover:bg-indigo-100 transition-all flex flex-col items-center gap-2 shadow-sm"><span className="text-3xl">🗜️</span> COMPOMAT G5</button>
               </div>
            </div>
          );
        }
        ...
        const docLinks = {
          'AMICUS': [{ t: 'Manual Técnico', l: 'https://drive.google.com/file/d/138n35pCNjruPm8z3rDk7sMwA3k_gKrml/view?usp=drive_link' }, { t: 'Manual Usuario', l: 'https://drive.google.com/file/d/1c8SfUfoQj4-S2drwPnu_oaQ27dokE5vB/view?usp=drive_link' }],
          'AMICORE': [{ t: 'Manual Técnico', l: 'https://drive.google.com/file/d/1SRK9tx68JT3OSvjboQWkIqnKVvKVU0Kx/view?usp=drive_link' }],
          'ALYX': [{ t: 'Manual Técnico', l: 'https://drive.google.com/file/d/1EbFMtnovi4Qs-D_Kk9LOa2sq74rFB6Hr/view?usp=drive_link' }],
          'COMPOMAT G5': [{ t: 'Manual Técnico', l: 'https://drive.google.com/file/d/1Rzm1Iso5UIv3zVc4ue6ml0Xv2cJ1bkS5/view?usp=drive_link' }],
          'COMPOGUARD': [{ t: 'Manual Técnico', l: 'https://drive.google.com/file/d/1QWjsUoxtVbh49-rUUXhGXrRrNwt3_5F0/view?usp=drive_link' }]
        };
        ...
        if (documentationSubScreen && docLinks[documentationSubScreen]) {
          return (
            <div className="p-6 bg-white border rounded-[2rem] text-left animate-fadeIn shadow-sm pb-20">
              <button onClick={goBack} className="mb-8 flex items-center gap-2 text-blue-600 font-black uppercase text-[10px] tracking-widest"><span className="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center">←</span> Volver</button>
              ...
              <div className="grid gap-3">
                {docLinks[documentationSubScreen].map(d => (
                  <button key={d.t} onClick={() => window.open(d.l, '_blank')} className="p-5 bg-blue-50 border-2 border-blue-100 rounded-2xl font-black text-blue-900 text-left hover:bg-blue-100 transition-all flex items-center gap-4">
                    <span className="text-xl">📄</span><span className="text-[11px] uppercase tracking-tighter leading-tight">{d.t}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="p-2 flex flex-col animate-fadeIn pb-20">
            <div className="grid grid-cols-2 gap-3 pb-10">
              {Object.keys(docLinks).map(k => (
                <button key={k} onClick={() => navigate('menu', 'materialApoyo', k)} className="p-6 bg-white border-2 border-gray-100 rounded-[2rem] text-[10px] font-black uppercase tracking-widest text-gray-700 shadow-sm hover:border-blue-500 transition-all flex flex-col items-center gap-3">
                  <div className="w-10 h-10 bg-blue-50 rounded-full flex items-center justify-center text-blue-600">📄</div>{k}
                </button>
              ))}
              <button onClick={() => navigate('menu', 'materialApoyo', 'SPAREPARTS')} className="p-6 bg-indigo-600 border-2 border-indigo-600 rounded-[2rem] text-[10px] font-black uppercase text-white shadow-xl col-span-2">🛠️ SPARE PARTS HUB</button>
            </div>
          </div>
        );
      ...
      case 'ordenesAsignadas':
          ...
          return (
            <div className="flex flex-col bg-white border rounded-[2rem] overflow-hidden animate-fadeIn pb-20 shadow-sm text-left">
              <div className="p-5 bg-blue-50 border-b flex items-center">
                <button onClick={goBack} className="mr-4 w-8 h-8 flex items-center justify-center bg-white rounded-lg text-blue-600 shadow-sm">←</button>
                <h3 className="font-black text-blue-900 tracking-tight uppercase text-xs">Detalle</h3>
              </div>
              <div className="p-6 space-y-6 overflow-y-auto min-h-[500px]">
                <div className="flex justify-between items-start">
                  <div><p className="text-[10px] font-black text-blue-500 uppercase">Servicio</p><p className="text-xl font-black text-gray-800">{so.tipoOS}</p></div>
                  <span className={`text-[10px] font-black px-4 py-2 rounded-xl shadow-sm ${so.status?.toLowerCase().includes('abierto') ? 'bg-orange-100 text-orange-600' : 'bg-green-100 text-green-600'}`}>{so.status?.toUpperCase() || 'ABIERTO'}</span>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100"><p className="text-[8px] font-black text-gray-400 uppercase">Falla</p><p className="text-[11px] font-bold text-gray-700 italic">"{so.falla}"</p></div>
                {so.status?.toLowerCase().includes('abierto') && (
                  <button onClick={() => closeOrder(so)} disabled={closingOrder} className="w-full bg-red-600 text-white font-black py-5 rounded-2xl shadow-xl uppercase">{closingOrder ? 'Procesando...' : 'Cerrar Orden'}</button>
                )}
              </div>
            </div>
          );
        ...
        return (
          <div className="flex flex-col bg-white border rounded-[2rem] overflow-hidden animate-fadeIn pb-20">
            <div className="p-5 bg-blue-50 border-b flex items-center">
                <button onClick={goBack} className="mr-4 w-8 h-8 flex items-center justify-center bg-white rounded-lg text-blue-600 shadow-sm">←</button>
                <h3 className="font-black text-blue-900 tracking-tight uppercase">Historial</h3>
            </div>
            <div className="flex-1 overflow-auto p-5 space-y-4 min-h-[400px]">
              {myOrders.length > 0 ? myOrders.map(so => (
                <div key={so.id} onClick={() => setSelectedOrderDetails(so)} className="p-5 border-l-[10px] border-l-blue-500 border rounded-[1.5rem] bg-gray-50 text-left shadow-sm active:scale-95 transition-all">
                  ...
                  <p className="text-[10px] text-gray-400 uppercase truncate">{so.cliente}</p>
                </div>
              )) : <div className="p-10 text-center font-bold text-gray-300 uppercase text-[10px]">Sin órdenes</div>}
            </div>
          </div>
        );
      default: return null;
    ...
    <div className="w-full relative h-full flex flex-col">
      <div className="flex justify-between items-center mb-6 border-b-2 border-gray-50 pb-5">
        {menuSubScreen === 'dashboard' ? (
          ...
          <div className="flex items-center gap-4"><button onClick={goBack} className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 font-black flex items-center justify-center shadow-sm active:scale-90 transition-all">←</button><h2 className="font-black uppercase text-[10px] tracking-widest text-gray-300">{menuSubScreen}</h2></div>
        )}
        {menuSubScreen === 'dashboard' && (<button onClick={() => navigate('landing')} className="w-10 h-10 flex items-center justify-center bg-red-50 text-red-500 rounded-xl active:bg-red-100 transition-all"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg></button>)}
      </div>
...
export default function App() {
  ...
  // --- EFECTO DE LIMPIEZA DE MENSAJES ---
  useEffect(() => {
    if (error || message) {
  ...
  // --- CONTROL DEL BOTÓN ATRÁS (BROWSER Y FÍSICO) ---
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubOrders = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'service_orders'), s => setSheetOrders(s.docs.map(d => ({id: d.id, ...d.data()}))));
    return () => { unsubContacts(); unsubOrders(); };
  }, [user]);

  const loginEngineer = async () => {
    if (!loginForm.engineerName || !loginForm.password) { setError("PIN REQUERIDO."); return; }
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4 font-sans overflow-hidden">
      <div className="w-full max-w-[420px] bg-white p-7 rounded-[4rem] shadow-2xl border-8 border-white relative min-h-[850px] max-h-[95vh] flex flex-col">
        <div className="absolute top-8 left-0 right-0 px-8 z-50 pointer-events-none text-center">
          ...
          {message && <div className="bg-green-500 text-white p-4 text-[10px] rounded-2xl mb-2 font-black shadow-lg animate-popIn uppercase tracking-widest">{message}</div>}
        </div>
        <div className="flex-1 overflow-y-auto scroll-smooth">
          {screen === 'landing' && <LandingScreen navigate={navigate} />}
          {screen === 'register' && <RegisterScreen form={registerForm} onChange={e => setRegisterForm({...registerForm, [e.target.name]: e.target.value})} onSubmit={registerEngineer} goBack={goBack} />}
          {screen === 'login' && <LoginScreen form={loginForm} onChange={e => setLoginForm({...loginForm, [e.target.name]: e.target.value})} onSubmit={loginEngineer} goBack={goBack} />}
          {screen === 'menu' && (
            <MenuScreen 
              menuSubScreen={menuSubScreen} setMenuSubScreen={(val) => navigate('menu', val)} 
              loginForm={loginForm} setScreen={setScreen} setMessage={setMessage} setError={setError} userId={user?.uid} db={db} contacts={contacts} 
              addContact={c => addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'contacts'), {...c, addedBy: user?.uid})} 
              deleteContact={id => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'contacts', id))} sheetOrders={sheetOrders} setSheetOrders={setSheetOrders} equipment={equipment} 
